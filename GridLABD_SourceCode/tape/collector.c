...
int read_aggregates(struct collector *my, AGGREGATION *aggr, char *buffer, int size)
{
	AGGREGATION *p;
	int offset=0;
	int count=0;
	...
	for (p=aggr; p!=NULL && offset<size-33; p=p->next)
	{
		...
		my->collectorValue = gl_run_aggregate(p); /* SJC edit to make transfering value to matlab easier */
...
EXPORT TIMESTAMP sync_collector(OBJECT *obj, TIMESTAMP t0, PASSCONFIG pass)
{
	...
	if(my->aggr != NULL && (my->interval == 0 || my->interval == -1)){
		if(read_aggregates(my,my->aggr,buffer,sizeof(buffer))==0)
		{
