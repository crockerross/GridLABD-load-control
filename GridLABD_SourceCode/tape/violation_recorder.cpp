...
int violation_recorder::init(OBJECT *obj){
	...
	make_object_list(FT_CLASS, "transformer", xfrmr_obj_list=new vobjlist());
	make_object_list(FT_CLASS, "overhead_line", ohl_obj_list=new vobjlist());
	make_object_list(FT_CLASS, "underground_line", ugl_obj_list=new vobjlist());
	make_object_list(FT_CLASS, "triplex_line", tplxl_obj_list=new vobjlist());
	make_object_list(FT_CLASS, "node", node_obj_list=new vobjlist());
	make_object_list(FT_CLASS, "triplex_node", tplx_node_obj_list=new vobjlist());
	make_object_list(FT_CLASS, "triplex_meter", tplx_mtr_obj_list=new vobjlist());
	make_object_list(FT_GROUPID, "Commercial_Triplex_Meter", tplx_mtr_comm_obj_list=new vobjlist());
	make_object_list(FT_GROUPID, "Residential_Triplex_Meter", tplx_mtr_res_obj_list=new vobjlist());
	make_object_list(FT_GROUPID, "Commercial_Meter", comm_mtr_obj_list=new vobjlist()); //This will overcount because some of them are also triplex_meter nodes
	make_object_list(FT_CLASS, "inverter", inverter_obj_list=new vobjlist()); // this could catch all inverters, so...
	//make_object_list(FT_MODULE, "powerflow", powerflow_obj_list=new vobjlist()); 
	// --> SJC edit, this function resulted in a stack overflow error when running the R3_1247_3 feeder (I think b/c it has so many nodes/components)
	// the list is used in the 2 functions below, but I don't think those functions are critical to the violation recorder.
	...
	xfrmr_list_v1 = new uniqueList();
	ohl_list_v1 = new uniqueList();
	ugl_list_v1 = new uniqueList();
	tplxl_list_v1 = new uniqueList();
	node_list_v2 = new uniqueList();
	tplx_node_list_v2 = new uniqueList();
	tplx_meter_comm_list_v2 = new uniqueList();  //SJC edit
	tplx_meter_res_list_v2 = new uniqueList();  //SJC edit
	comm_meter_list_v2 = new uniqueList();
	node_list_v3 = new uniqueList();
	tplx_node_list_v3 = new uniqueList();
	tplx_meter_list_v3 = new uniqueList();
	tplx_meter_list_v3 = new uniqueList();
	tplx_meter_comm_list_v3 = new uniqueList();
	tplx_meter_res_list_v3 = new uniqueList();
	comm_meter_list_v3 = new uniqueList();
	inverter_list_v6 = new uniqueList();
	tplx_meter_list_v7 = new uniqueList();
	comm_meter_list_v7 = new uniqueList();
...
int violation_recorder::write_header(){
	...
	if(0 > fprintf(rec_file,"# timestamp, violation, observation, upper_limit, lower_limit, nominal value, object(s), object type(s), phase, message\n")){ return 0; }
	...
	for(curr = list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S)) { // split phase line
			if (fails_static_condition(curr->obj, "current_out_A", upper_bound, lower_bound, nominal, &retval)) {
				...
				write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, S1, Current violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
			if (fails_static_condition(curr->obj, "current_out_B", upper_bound, lower_bound, nominal, &retval)) {
				uniq_list->insert(curr->obj->name);
				increment_violation(VIOLATION1, type);
				write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, S2, Current violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
			...
			if (has_phase(curr->obj, PHASE_A)) {
				if (fails_static_condition(curr->obj, "current_out_A", upper_bound, lower_bound, nominal, &retval)) {
					uniq_list->insert(curr->obj->name);
					increment_violation(VIOLATION1, type);
					write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, A, Current violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_B)) {
				if (fails_static_condition(curr->obj, "current_out_B", upper_bound, lower_bound, nominal, &retval)) {
					uniq_list->insert(curr->obj->name);
					increment_violation(VIOLATION1, type);
					write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, B, Current violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_C)) {
				if (fails_static_condition(curr->obj, "current_out_C", upper_bound, lower_bound, nominal, &retval)) {
					uniq_list->insert(curr->obj->name);
					increment_violation(VIOLATION1, type);
					write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, C, Current violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
		}
	}
...
int violation_recorder::check_xfrmr_thermal_limit(TIMESTAMP t1, vobjlist *list, uniqueList *uniq_list, int type, double upper_bound, double lower_bound) {
	...
	for(curr = list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S)) {
			...
			//p_ptr = gl_get_property(curr->obj, "continuous_rating");
			//nominal = get_observed_double_value(curr->obj, p_ptr)*1000.;
			if (fails_static_condition(curr->obj, "power_out", upper_bound, lower_bound, nominal, &retval)) {
				uniq_list->insert(curr->obj->name);
				increment_violation(VIOLATION1, type);
				write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, S, Power violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
		// this is for the other transformers which have 3 phases, each of which can violate the limit
			...
			if (has_phase(curr->obj, PHASE_A)) {
				...
				if (fails_static_condition(curr->obj, "power_out_A", upper_bound, lower_bound, nominal, &retval)) {
					uniq_list->insert(curr->obj->name);
					increment_violation(VIOLATION1, type);
					write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, A, Power violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_B)) {
				...
				if (fails_static_condition(curr->obj, "power_out_B", upper_bound, lower_bound, nominal, &retval)) {
					uniq_list->insert(curr->obj->name);
					increment_violation(VIOLATION1, type);
					write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, B, Power violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_C)) {
				...
				if (fails_static_condition(curr->obj, "power_out_C", upper_bound, lower_bound, nominal, &retval)) {
					uniq_list->insert(curr->obj->name);
					increment_violation(VIOLATION1, type);
					write_to_stream(t1, echo, "VIOLATION1, %f, %f, %f, %f, %s, %s, C, Power violates thermal limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
		}
	}
	...
	for(curr = node_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_A)) {
			if (fails_static_condition(curr->obj, "voltage_A", node_upper_bound, node_lower_bound, nominal, &retval)) {
				...
				write_to_stream(t1, echo, "VIOLATION2, %f, %f, %f, %f, %s, %s, A, Per unit voltage violates limit.", retval, node_upper_bound, node_lower_bound,nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
		}
		if (has_phase(curr->obj, PHASE_B)) {
			if (fails_static_condition(curr->obj, "voltage_B", node_upper_bound, node_lower_bound, nominal, &retval)) {
				node_list_v2->insert(curr->obj->name);
				increment_violation(VIOLATION2, NODE);
				write_to_stream(t1, echo, "VIOLATION2, %f, %f, %f, %f, %s, %s, B, Per unit voltage violates limit.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
		}
		if (has_phase(curr->obj, PHASE_C)) {
			if (fails_static_condition(curr->obj, "voltage_C", node_upper_bound, node_lower_bound, nominal, &retval)) {
				node_list_v2->insert(curr->obj->name);
				increment_violation(VIOLATION2, NODE);
				write_to_stream(t1, echo, "VIOLATION2, %f, %f, %f, %f, %s, %s, C, Per unit voltage violates limit.", retval, node_upper_bound, node_lower_bound,nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
		}
	}
	...
	for(curr = tplx_node_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2)) {
			if (fails_static_condition(curr->obj, "voltage_12", node_upper_bound, node_lower_bound, nominal, &retval)) {
				...
				write_to_stream(t1, echo, "VIOLATION2, %f, %f, %f, %f, %s, %s, S, Per unit voltage violates limit.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
			}
		}
	}
	
	for(curr = tplx_mtr_comm_obj_list; curr != 0; curr = curr->next){
		if (curr->obj == 0) continue;
			p_ptr = gl_get_property(curr->obj, "nominal_voltage");
			nominal = get_observed_double_value(curr->obj, p_ptr) * 2.;
			if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2)) {
				if (fails_static_condition(curr->obj, "voltage_12", node_upper_bound, node_lower_bound, nominal, &retval)) {
					tplx_meter_comm_list_v2->insert(curr->obj->name);
					increment_violation(VIOLATION2, TPXMC);
					write_to_stream(t1, echo, "VIOLATION2, %f, %f, %f, %f, %s, %s, S, Per unit voltage violates limit.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
	}

	for(curr = tplx_mtr_res_obj_list; curr != 0; curr = curr->next){
		if (curr->obj == 0) continue;
			p_ptr = gl_get_property(curr->obj, "nominal_voltage");
			nominal = get_observed_double_value(curr->obj, p_ptr) * 2.;
			if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2)) {
				if (fails_static_condition(curr->obj, "voltage_12", node_upper_bound, node_lower_bound, nominal, &retval)) {
					tplx_meter_res_list_v2->insert(curr->obj->name);
					increment_violation(VIOLATION2, TPXMR);
					write_to_stream(t1, echo, "VIOLATION2, %f, %f, %f, %f, %s, %s, S, Per unit voltage violates limit.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name);
				}
			}
	}
...
//	gl_output("VIOLATION 3");
	...
	for(curr = node_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_A)) {
			if (fails_continuous_condition(curr,0, "voltage_A", t1, interval, node_upper_bound, node_lower_bound, nominal, &retval)) {
				node_list_v3->insert(curr->obj->name);
				increment_violation(VIOLATION3, NODE);
				write_to_stream(t1, echo, "VIOLATION3, %f, %f, %f, %f, %s, %s, A, Per unit voltage violates limit continuously over %is interval.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)node_continuous_voltage_interval);
			}
		}
		if (has_phase(curr->obj, PHASE_B)) {
			if (fails_continuous_condition(curr,0, "voltage_B", t1, interval, node_upper_bound, node_lower_bound, nominal, &retval)) {
				node_list_v3->insert(curr->obj->name);
				increment_violation(VIOLATION3, NODE);
				write_to_stream(t1, echo, "VIOLATION3, %f, %f, %f, %f, %s, %s, B, Per unit voltage violates limit continuously over %is interval.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)node_continuous_voltage_interval);
			}
		}
		if (has_phase(curr->obj, PHASE_C)) {
			if (fails_continuous_condition(curr,0, "voltage_C", t1, interval, node_upper_bound, node_lower_bound, nominal, &retval)) {
				node_list_v3->insert(curr->obj->name);
				increment_violation(VIOLATION3, NODE);
				write_to_stream(t1, echo, "VIOLATION3, %f, %f, %f, %f, %s, %s, C, Per unit voltage violates limit continuously over %is interval.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)node_continuous_voltage_interval);
			}
		}
	}
	...
	for(curr = tplx_node_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2)) { // We are now checking all objects regardless of parent // && gl_object_isa(curr->obj->parent,"transformer")) {
			if (fails_continuous_condition(curr, 0, "voltage_12", t1, interval, node_upper_bound, node_lower_bound, nominal, &retval)) {
				...
				write_to_stream(t1, echo, "VIOLATION3, %f, %f, %f, %f, %s, %s, S, Per unit voltage violates limit continuously over %is interval.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)node_continuous_voltage_interval);
			}
		}
	}
	...
	for(curr = tplx_mtr_comm_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2)) { // We are now checking all objects regardless of parent // && gl_object_isa(curr->obj->parent,"transformer")) {
			if (fails_continuous_condition(curr, 0, "voltage_12", t1, interval, node_upper_bound, node_lower_bound, nominal, &retval)) {
				tplx_meter_comm_list_v3->insert(curr->obj->name);
				increment_violation(VIOLATION3, TPXMC);
				write_to_stream(t1, echo, "VIOLATION3, %f, %f, %f, %f, %s, %s, S, Per unit voltage violates limit continuously over %is interval.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)node_continuous_voltage_interval);
			}
		}
	}
	...
	for(curr = tplx_mtr_res_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S1) && has_phase(curr->obj, PHASE_S2)) { // We are now checking all objects regardless of parent // && gl_object_isa(curr->obj->parent,"transformer")) {
			if (fails_continuous_condition(curr, 0, "voltage_12", t1, interval, node_upper_bound, node_lower_bound, nominal, &retval)) {
				tplx_meter_res_list_v3->insert(curr->obj->name);
				increment_violation(VIOLATION3, TPXMR);
				write_to_stream(t1, echo, "VIOLATION3, %f, %f, %f, %f, %s, %s, S, Per unit voltage violates limit continuously over %is interval.", retval, node_upper_bound, node_lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)node_continuous_voltage_interval);
			}
		}
	}
...
//	gl_output("%s", violation_name);
	...
	if (has_phase(link_monitor_obj, PHASE_A) && ((*directions & AR) == AR)) {
		if (fails_static_condition(link_monitor_obj, "current_out_A", breaker_limit_A_upper, breaker_limit_A_lower, nominal, &retval)) {
			...
			write_to_stream(t1, echo, "VIOLATION%i, %f, %f, %f, %s, %s, %s, A, %s", (int)l2((double)violation_num)+1, retval, breaker_limit_A_upper, breaker_limit_A_lower, "n/a", gl_name(link_monitor_obj, objname, 127), link_monitor_obj->oclass->name, str);
		}
	}
	if (has_phase(link_monitor_obj, PHASE_B) && ((*directions & BR) == BR)) {
		if (fails_static_condition(link_monitor_obj, "current_out_B", breaker_limit_B_upper, breaker_limit_B_lower, nominal, &retval)) {
			increment_violation(violation_num);
			write_to_stream(t1, echo, "VIOLATION%i, %f, %f, %f, %s, %s, %s, B, %s",  (int)l2((double)violation_num)+1, retval, breaker_limit_B_upper, breaker_limit_B_lower, "n/a", gl_name(link_monitor_obj, objname, 127), link_monitor_obj->oclass->name, str);
		}
	}
	if (has_phase(link_monitor_obj, PHASE_C) && ((*directions & CR) == CR)) {
		if (fails_static_condition(link_monitor_obj, "current_out_C", breaker_limit_C_upper, breaker_limit_C_lower, nominal, &retval)) {
			increment_violation(violation_num);
			write_to_stream(t1, echo, "VIOLATION%i, %f, %f, %f, %s, %s, %s, C, %s",  (int)l2((double)violation_num)+1, retval, breaker_limit_C_upper, breaker_limit_C_lower, "n/a", gl_name(link_monitor_obj, objname, 127), link_monitor_obj->oclass->name, str);
		}
	}
	...
	for(curr = inverter_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_S)) { // inverter connected to a triplex system, only checking one phase here
			if (fails_dynamic_condition(curr, 1, "phaseB_V_Out", t1, interval, upper_bound, lower_bound, nominal, &retval)) { // this is S1 !?!
				...
				write_to_stream(t1, echo, "VIOLATION6, %f, %f, %f, %f, %s, %s, S, Voltage change between %is intervals violates limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)inverter_v_chng_interval);
			}
			...
			if (has_phase(curr->obj, PHASE_A)) {
				if (fails_dynamic_condition(curr, 0, "phaseA_V_Out", t1, interval, upper_bound, lower_bound, nominal, &retval)) {
					inverter_list_v6->insert(curr->obj->name);
					increment_violation(VIOLATION6);
					write_to_stream(t1, echo, "VIOLATION6, %f, %f, %f, %f, %s, %s, A, Voltage change between %is intervals violates limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)inverter_v_chng_interval);
				}
			}
			if (has_phase(curr->obj, PHASE_B)) {
				if (fails_dynamic_condition(curr, 1, "phaseB_V_Out", t1, interval, upper_bound, lower_bound, nominal, &retval)) {
					inverter_list_v6->insert(curr->obj->name);
					increment_violation(VIOLATION6);
					write_to_stream(t1, echo, "VIOLATION6, %f, %f, %f, %f, %s, %s, B, Voltage change between %is intervals violates limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)inverter_v_chng_interval);
				}
			}
			if (has_phase(curr->obj, PHASE_C)) {
				if (fails_dynamic_condition(curr, 2, "phaseC_V_Out", t1, interval, upper_bound, lower_bound, nominal, &retval)) {
					inverter_list_v6->insert(curr->obj->name);
					increment_violation(VIOLATION6);
					write_to_stream(t1, echo, "VIOLATION6, %f, %f, %f, %f, %s, %s, C, Voltage change between %is intervals violates limit.", retval, upper_bound, lower_bound, nominal, gl_name(curr->obj, objname, 127), curr->obj->oclass->name, (int)inverter_v_chng_interval);
				}
			}
		}
	}
		...
		if (has_phase(curr->obj, PHASE_S1)) {
			...
			if (has_phase(curr->obj, PHASE_A)) {
				...
				if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
					...
					write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, A S1, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_B)) {
				p_ptr = gl_get_property(curr->ref_obj, "nominal_voltage");
				...
				if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
					tplx_meter_list_v7->insert(curr->obj->name);
					increment_violation(VIOLATION7, TPXM);
					write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, B S1, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_C)) {
				p_ptr = gl_get_property(curr->ref_obj, "nominal_voltage");
				...
				if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
					tplx_meter_list_v7->insert(curr->obj->name);
					increment_violation(VIOLATION7, TPXM);
					write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, C S1, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
				}
			}
		}
		if (has_phase(curr->obj, PHASE_S2)) {
			...
			if (has_phase(curr->obj, PHASE_A)) {
				...
				if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
					tplx_meter_list_v7->insert(curr->obj->name);
					increment_violation(VIOLATION7, TPXM);
					write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, A S2, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_B)) {
				p_ptr = gl_get_property(curr->ref_obj, "nominal_voltage");
				...
				if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
					tplx_meter_list_v7->insert(curr->obj->name);
					increment_violation(VIOLATION7, TPXM);
					write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, B S2, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
				}
			}
			if (has_phase(curr->obj, PHASE_C)) {
				p_ptr = gl_get_property(curr->ref_obj, "nominal_voltage");
				...
				if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
					tplx_meter_list_v7->insert(curr->obj->name);
					increment_violation(VIOLATION7, TPXM);
					write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, C S2, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
				}
			}
		}
	}
	...
	for(curr = comm_mtr_obj_list; curr != 0; curr = curr->next){
		...
		if (has_phase(curr->obj, PHASE_A)) {
			...
			if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
				...
				write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, A, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
			}
		}
		if (has_phase(curr->obj, PHASE_B)) {
			...
			if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
				comm_meter_list_v7->insert(curr->obj->name);
				increment_violation(VIOLATION7, CMTR);
				write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, B, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
			}
		}
		if (has_phase(curr->obj, PHASE_C)) {
			...
			if (fails_static_condition(pu, pu_upper_bound, pu_lower_bound, 1.0, &retval)) {
				comm_meter_list_v7->insert(curr->obj->name);
				increment_violation(VIOLATION7, CMTR);
				write_to_stream(t1, echo, "VIOLATION7, %f, %f, %f, %f, %s %s, %s %s, C, Per unit voltage difference between objects violates limit.", retval, pu_upper_bound, pu_lower_bound, xfrmr_nominal, gl_name(curr->obj, metername, 127), gl_name(curr->ref_obj, xfrmrname, 127), curr->obj->oclass->name, curr->ref_obj->oclass->name);
			}
		}
	}
...
int violation_recorder::check_violation_8(TIMESTAMP t1) {
	...
	if (fails_static_condition (pf, 1.0, substation_pf_lower_limit, 1.0, &retval)) {
		...
		write_to_stream(t1, echo, "VIOLATION8, %f, %f, %f, %f, %s, %s,, %s", retval, 1.0, substation_pf_lower_limit, nominal, gl_name(link_monitor_obj, objname, 127), link_monitor_obj->oclass->name, "Power factor violates limit.");
	}
...
int violation_recorder::write_summary() {
	...
	if(0 > fprintf(f,"    TRIPLEX METER COMMERCIAL (%i of %i meters in violation),%i\n", tplx_meter_comm_list_v2->length(), tplx_mtr_comm_obj_list->length(), get_violation_count(VIOLATION2,TPXMC))){ return 0; }
	if(0 > fprintf(f,"    TRIPLEX METER RESIDENTIAL (%i of %i meters in violation),%i\n", tplx_meter_res_list_v2->length(), tplx_mtr_res_obj_list->length(), get_violation_count(VIOLATION2,TPXMR))){ return 0; }
	...
	if(0 > fprintf(f,"    NODE (%i of %i nodes in violation),%i\n", node_list_v3->length(), node_obj_list->length(), get_violation_count(VIOLATION3,NODE))){ return 0; }
	if(0 > fprintf(f,"    TRIPLEX NODE (%i of %i nodes in violation),%i\n", tplx_node_list_v3->length(), tplx_node_obj_list->length(), get_violation_count(VIOLATION3,TPXN))){ return 0; }
	if(0 > fprintf(f,"    TRIPLEX METER COMMERCIAL (%i of %i meters in violation),%i\n", tplx_meter_comm_list_v3->length(), tplx_mtr_comm_obj_list->length(), get_violation_count(VIOLATION3,TPXMC))){ return 0; }
	if(0 > fprintf(f,"    TRIPLEX METER RESIDENTIAL (%i of %i meters in violation),%i\n", tplx_meter_res_list_v3->length(), tplx_mtr_res_obj_list->length(), get_violation_count(VIOLATION3,TPXMR))){ return 0; }
