...
house_e::house_e(MODULE *mod) : residential_enduse(mod)
{
	...
	if (oclass==NULL)  
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_double,"hvac_onAndAvailable",PADDR(hvac_onAndAvailable),PT_DESCRIPTION,"hvac is ON, zero if hvac is unavailable for switching (either outside of thermostat range or b/c of lockout time), 1 otherwise",
			PT_double,"hvac_offAndAvailable",PADDR(hvac_offAndAvailable),PT_DESCRIPTION,"hvac is OFF, zero if hvac is unavailable for switching (either outside of thermostat range or b/c of lockout time), 1 otherwise",
			PT_double,"locked_out",PADDR(locked_out),PT_DESCRIPTION,"hvac is not able to switch because of a lockout time",
			...
			PT_bool, "first_cool_on", PADDR(first_cool_on), PT_DESCRIPTION,"indicates that cooling system has turned on `naturally'",
...
int house_e::create() 
{
	...
	thermostat_off_cycle_time = 120;
	thermostat_on_cycle_time = 120;
	hvac_onAndAvailable = 0;
	hvac_offAndAvailable = 0;
	locked_out = 0;
...
int house_e::init(OBJECT *parent)
{
	...
	if (thermostat_cycle_time<=0.0) thermostat_cycle_time = 0; // this used to 120 seconds
	if (Tair==0.0){
	...
	first_cool_on = false; //SJCR added
		...
		case SM_COOL:
			...
			switch(cooling_system_type){
				...
				case CT_ELECTRIC:
					...
					system_rated_capacity_lastOn = system_rated_capacity; //SJCR added 9/12/17
					break;
			}
			break;
...
//			gl_debug("house %s (%d) sync_panel event '%s'", obj->name, obj->id, gl_strftime(t2));
	...
	/* SJC commented block 3/1/17 so that the thermostat is always checked (makes temperature control override lockout time)
	if(thermostat_off_cycle_time == -1 && thermostat_on_cycle_time == -1){
		...
		if(thermostat_last_off_cycle_time > thermostat_last_on_cycle_time){ // if thermostat switched OFF most recently
...
TIMESTAMP house_e::sync_thermostat(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	double saveHeatgain = 0; //SJCR 9/10
	double saveTotalHeatgain = 0; //SJCR 9/10
	double dt2_alt = 0;
	...
	if(thermostat_off_cycle_time == -1 && thermostat_on_cycle_time == -1){
		if(t1 < thermostat_last_cycle_time + thermostat_cycle_time){
			locked_out = 1;
// SJC 3/1/17			return (thermostat_last_cycle_time + thermostat_cycle_time); // next time will be calculated in sync_model
		}
	} else if(thermostat_off_cycle_time >=0 && thermostat_on_cycle_time >=0){
		if(thermostat_last_off_cycle_time > thermostat_last_on_cycle_time){
			if(t1 < thermostat_last_off_cycle_time + thermostat_off_cycle_time){
				locked_out = 1;
// SJC 3/1/17				return (thermostat_last_off_cycle_time + thermostat_off_cycle_time); // next time will be calculated in sync_model
			}
		} else if(thermostat_last_off_cycle_time < thermostat_last_on_cycle_time){
			if(t1 < thermostat_last_on_cycle_time + thermostat_on_cycle_time){
				locked_out = 1;
// SJC 3/1/17				return (thermostat_last_on_cycle_time + thermostat_on_cycle_time); // next time will be calculated in sync_model
			} 
		} else {
			if(t1 < thermostat_last_cycle_time + thermostat_cycle_time){
				locked_out = 1;
// SJC 3/1/17				return (thermostat_last_cycle_time + thermostat_cycle_time); // next time will be calculated in sync_model
	...
	hvac_onAndAvailable = 0;   // added by SJC
	hvac_offAndAvailable = 0;
	if (thermostat_control!=TC_NONE && first_cool_on == true){
		if (system_mode == SM_COOL) {
			if (locked_out == 0) {
				// 9/10 added by SJCR {
				saveHeatgain = load.heatgain; //this is the "heat"gain of the HVAC unit
				saveTotalHeatgain = total.heatgain; // total heatgain of all loads in house (includes gain from HVAC)
				total.heatgain = total.heatgain - load.heatgain;
				load.heatgain = 0;
				update_model(); // recomputes thermal parameters (k1,r1,k2,r2,(Teq?))
				dt2_alt = e2solve(k1,r1,k2,r2,Teq-TcoolOn)*3600.0; // finds the time until HVAC unit hits TcoolOn IF it is forced off
				// return states back to what they were
				load.heatgain = saveHeatgain;
				total.heatgain = saveTotalHeatgain;
				update_model();
				//}end addition 
				if (Tair > (TcoolOff) && Tair < (TcoolOn) && dt2_alt>120) {
					hvac_onAndAvailable = 1;
				} 
			}
		} else if (system_mode == SM_OFF) {
			if (locked_out == 0) {
				// 9/10 added by SJCR {
				saveHeatgain = load.heatgain;
				saveTotalHeatgain = total.heatgain;
				load.heatgain = system_rated_capacity_lastOn;
				total.heatgain = total.heatgain + load.heatgain;
				update_model();
				dt2_alt = e2solve(k1,r1,k2,r2,Teq-TcoolOff)*3600.0;
				// return states back to what they were
				load.heatgain = saveHeatgain;
				total.heatgain = saveTotalHeatgain;
				update_model();
	...
	// re_override is the command from the tracking control, produced by the function calc_probonoff in the passive_controller.cpp
	if (this->re_override == OV_ON && hvac_offAndAvailable == 1){
		TcoolOn = TcoolOff; // removed 3/1 + 0.1*thermostat_deadband; // SCR added the addition of 10% thermostat deadband 10/11
		TheatOn = TheatOff;
		if(dlc_offset == 0.0){
			//re_override = OV_NORMAL;
		}
//		thermostat_last_cycle_time = gl_globalclock - thermostat_cycle_time - 1;
	} else if(this->re_override == OV_OFF && hvac_onAndAvailable == 1){
		if (dlc_offset == 0.0)
		{
			TcoolOff = TcoolOn; // removed 3/1 - 0.1*thermostat_deadband;  // SCR added the subtraction of 10% thermostat deadband 10/11
			TheatOff = TheatOn;
			//re_override = OV_NORMAL;
		}
	...
	*/
	...
	if (thermostat_control!=TC_NONE)
	{
		...
		case SM_HEAT:
			...
			break;
		case SM_AUX:
			if(Tair > TheatOff - terr/2 ){ // && re_override != OV_ON removed by SJC
				system_mode = SM_OFF;
				power_state = PS_OFF;
				thermostat_last_cycle_time = t1;
				thermostat_last_off_cycle_time = t1;
			...
			break;
		case SM_COOL:
			if(Tair < TcoolOff - terr/2 ){ // && re_override != OV_ON removed by SJC
				system_mode = SM_OFF;
				power_state = PS_OFF;
				thermostat_last_cycle_time = t1;
				thermostat_last_off_cycle_time = t1;
