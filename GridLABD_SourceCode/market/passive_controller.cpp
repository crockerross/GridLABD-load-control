...
/**/		PT_bool,"input_chained",PADDR(input_chained),
			...
			PT_double,"prob_onoff",PADDR(prob_onoff),PT_ACCESS,PA_REFERENCE,PT_DESCRIPTION,"the probability of switching on (>0) or off (<0)",
...
/**/		PT_double,"output_setpoint",PADDR(output_setpoint),
			...
			PT_enumeration,"control_mode",PADDR(control_mode),PT_DESCRIPTION,"the control mode to use for determining controller action",
				...
				PT_KEYWORD,"PROBABILITY_ONOFF",(enumeration)CM_PROBONOFF,
...
int passive_controller::init(OBJECT *parent){
	...
	if(parent == NULL){
		...
		// output_setpoint
		if (control_mode != this->CM_PROBOFF && control_mode != this->CM_PROBONOFF && control_mode != this->CM_ELASTICITY_MODEL && control_mode != this->CM_DLC)
		{
	...
	if (pool_pump_model == false && control_mode != CM_ELASTICITY_MODEL && control_mode != CM_DLC && control_mode != CM_PROBONOFF){
	...
	if(last_cycle == 0 || t1 >= last_cycle + period || period == 0){
		...
		switch(control_mode){
			...
			case CM_PROBOFF:
				...
				break;
			case CM_PROBONOFF:
				if(calc_probonoff(t0, t1) != 0){
					GL_THROW("error occured when handling the probabilistic on-off control mode");
				}
				break;
			case CM_DUTYCYCLE:
...
int passive_controller::calc_proboff(TIMESTAMP t0, TIMESTAMP t1){
	...
	if(r < prob_off){
		output_state = -1; // if state is override, override = OV_OFF
	} else {
		output_state = 0; //override = OV_NORMAL
	}

	return 0;
}

int passive_controller::calc_probonoff(TIMESTAMP t0, TIMESTAMP t1){
	if(observation_addr == 0){
		GL_THROW("insufficient input for probabilistic shutoff control mode");
		return -1;
	}
	...
	switch(distribution_type){
		case PDT_UNIFORM:
			prob_onoff = comfort_level * observation;
			break;
		default:
			gl_error("");
	}
	if(!isfinite(prob_onoff)){
		return 0;
	}
	r = gl_random_uniform(RNGSTATE,0.0,1.0);
	if (prob_onoff > 0){
