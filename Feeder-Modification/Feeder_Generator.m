...
[LUSolverVal,taxonomy_files,taxonomy_directory,output_directory,my_region] = user_configuration('Steph');
load('transformer.mat')
initDataFilename = 'initData_R1_1247_1';
load(initDataFilename)
...
for tax_ind=1:no_of_tax 
    ...
    use_flags.use_market = 0; % 0 = NONE, 1 = BIDDING, 2 = PASSIVE (NON-BIDDING), 4 = PROB ON OFF
    ...
    if (use_flags.use_control ~=0 && use_flags.use_market~=0)
        error('Control and Market Incentive being used simultaneously');
    end
    if use_flags.use_rngSetPoints == 1 && use_flags.use_rng_allParam == 1
        error('Rng Set Points and Rng All Parameters are being used simultaneously');
    end
    
    altRng = RandStream('mt19937ar','seed',initData.altRngSeed);
    ...
    if use_flags.use_rng_allParam ==1
        [s1,s2,s3,s4,s5,s6] = RandStream.create('mrg32k3a','NumStreams',6,'seed',10);
    else [s1,s2,s3,s4,s5,s6] = RandStream.create('mrg32k3a','NumStreams',6);
    end
    ...
    for i=1:a
        ...
        elseif (strfind(char(test{3}{i}),'{') ~= 0)
            ...
            nn = nn + 1;
            if use_flags.use_xfmrThermalModel == 1  
                if (strcmp(char(test{2}{i}),'transformer') ~= 0)
                    xfmName = test{2}{i+1};
                    xfmNameData = strsplit(xfmName,'_');
                    xfmNum = char(xfmNameData(3));
                    xfmNum = strsplit(xfmNum,';');
                    xfmNum = char(xfmNum(1));
                    xfmNum = str2double(xfmNum);

                    if (strcmp(char(test{2}{i+2}),'AS;')~=0 || strcmp(char(test{2}{i+2}),'BS;')~=0 || strcmp(char(test{2}{i+2}),'CS;')~=0)
                        glm_final{1}{nn}= '     ';
                        glm_final{2}{nn}= 'use_thermal_model';
                        glm_final{3}{nn}= 'TRUE;';
                        glm_final{1}{nn+1}= '     ';
                        glm_final{2}{nn+1}= 'climate';
                        glm_final{3}{nn+1}= strcat(tmy,';');
                        glm_final{1}{nn+2}= '     ';
                        glm_final{2}{nn+2}='aging_granularity';
                        glm_final{3}{nn+2}= '60;';
                        glm_final{1}{nn+3}= '     ';
                        glm_final{2}{nn+3}='top_oil_hot_spot_temperature';
                        if use_flags.use_setInitTemps ==1 && isfield(initData, 'oilTemp')
                            glm_final{3}{nn+3}= strcat(num2str(initData.oilTemp(xfmNum)),';');
                        else glm_final{3}{nn+3}= '68;';
                        end
                        glm_final{1}{nn+4}= '     ';
                        glm_final{2}{nn+4}='winding_hot_spot_temperature';
                        if use_flags.use_setInitTemps ==1 && isfield(initData, 'coilTemp')
                            glm_final{3}{nn+4}= strcat(num2str(initData.coilTemp(xfmNum)),';');
                        else glm_final{3}{nn+4}='68;';
                        end
                        nn = nn+5;
                    end
                end
    ...
    for i=1:y
        ...
        elseif (strfind(char(config{2}{i}),'transformer_configuration:') ~= 0)
            ...
            mm = mm + 2;
            for ssss=i:(i+9)
                if (strfind('SINGLE_PHASE_CENTER_TAPPED;',char(config{2}{ssss}))>0)
                    singlePhase = 1;
                end 
                if strcmp('power_rating',config{1}{ssss})>0
                    powerRating = num2str(floor(str2double(strrep(config{2}{ssss},';',''))));
                end
            end
            if use_flags.use_xfmrThermalModel == 1 && singlePhase == 1 && strcmp(powerRating, '') == 0
                structName = sprintf('transformer.P%s',powerRating);
                config_final{1}{mm} ='     ';
                config_final{2}{mm} = 'oil_volume';
                config_final{3}{mm} = strcat(num2str(eval(strcat(structName, '.oil_volume'))),';');
                config_final{1}{mm+1} ='     ';
                config_final{2}{mm+1} = 'core_coil_weight';
                config_final{3}{mm+1} = strcat(num2str(eval(strcat(structName, '.core_coil_weight'))),';');
                config_final{1}{mm+2} ='     ';
                config_final{2}{mm+2} = 'tank_fittings_weight';
                config_final{3}{mm+2} = strcat(num2str(eval(strcat(structName, '.tank_fitting_weight'))),';');
                config_final{1}{mm+3} ='     ';
                config_final{2}{mm+3} = 'full_load_loss';
                config_final{3}{mm+3} = strcat(num2str(eval(strcat(structName, '.full_load_loss'))),';');
                config_final{1}{mm+4} ='     ';
                config_final{2}{mm+4} = 'no_load_loss';
                config_final{3}{mm+4} = strcat(num2str(eval(strcat(structName, '.no_load_loss'))),';');
                config_final{1}{mm+5} ='     ';
                config_final{2}{mm+5} = 'rated_winding_hot_spot_rise';
                config_final{3}{mm+5} = strcat(num2str(eval(strcat(structName, '.rated_winding_hot_spot_rise'))),';');
                config_final{1}{mm+6} ='     ';
                config_final{2}{mm+6} = 'rated_top_oil_rise';
                config_final{3}{mm+6} = strcat(num2str(eval(strcat(structName, '.rated_top_oil_rise'))),';');
                config_final{1}{mm+7} ='     ';
                config_final{2}{mm+7} = 'coolant_type';
                config_final{3}{mm+7} = strcat(eval(strcat(structName, '.coolant_type')),';');
                config_final{1}{mm+8} ='     ';
                config_final{2}{mm+8} = 'cooling_type';
                config_final{3}{mm+8} = strcat(eval(strcat(structName, '.cooling_type')),';');
                config_final{1}{mm+9} ='     ';
                config_final{2}{mm+9} = 'rated_winding_time_constant';
                config_final{3}{mm+9} = strcat(num2str(eval(strcat(structName, '.rated_winding_time_constant'))),';');
                config_final{1}{mm+10} ='     ';
                config_final{2}{mm+10} = 'installed_insulation_life';
                config_final{3}{mm+10} = '175200;';
                mm = mm+11;
            end
            singlePhase = 0;
            powerRating = '';
    ...
    fprintf(write_file,'#set randomseed=%d; \n',initData.altRngSeed);

  
    if (use_flags.use_homes ~= 0)
        fprintf(write_file,'#include ".\\Schedules\\appliance_schedules.glm";\n');
        fprintf(write_file,'#include ".\\Schedules\\water_and_setpoint_schedule_v5.glm";\n');
    end
    ...
    if (use_flags.use_batt == 1 || use_flags.use_batt == 2)
        fprintf(write_file,'#include ".\\Schedules\\battery_schedule.glm";\n');
    end
    if (use_flags.use_commercial == 1)
        fprintf(write_file,'#include ".\\Schedules\\commercial_schedules.glm";\n');
    end
    if (use_flags.use_market == 1 || use_flags.use_market == 2)
        fprintf(write_file,'#include ".\\Schedules\\daily_elasticity_schedules.glm";\n');
    end
    if ((use_flags.use_ts == 2) || (use_flags.use_ts == 4))
        fprintf(write_file,'#include ".\\Schedules\\thermal_storage_schedule_R%d.glm";\n',region);
    end

    fprintf(write_file,'//#define stylesheet=http://gridlab-d.svn.sourceforge.net/viewvc/gridlab-d/trunk/core/gridlabd-2_0\n');
    fprintf(write_file,'#set minimum_timestep=2;\n');
    fprintf(write_file,'#set profiler=1;\n');
    fprintf(write_file,'#set relax_naming_rules=1;\n\n');
    if use_flags.use_control == 1
        fprintf(write_file,'#set pauseatexit=TRUE\n');
        fprintf(write_file,'//link with matlab\n');
        fprintf(write_file,'#ifdef MATLAB\n');
        fprintf(write_file,'link matlabController.link;\n');
    ...
    if use_flags.use_control == 2
        
        fprintf(write_file,'object player {\n');
        fprintf(write_file,'     name setPointAC;\n');
        fprintf(write_file,'     file "acSetPointChange.player"; //"constant_SP.player";\n');
        fprintf(write_file,'     loop 10000;\n');
        fprintf(write_file,'};\n\n');

        fprintf(write_file,'object player {\n');
        fprintf(write_file,'     name setPointWaterHeater;\n');
        fprintf(write_file,'     file "setPointWaterHeater.player"; //"constant_SP.player";\n');
        fprintf(write_file,'     loop 10000;\n');
        fprintf(write_file,'};\n\n');

        fprintf(write_file,'object player {\n');
        fprintf(write_file,'     name poolDutyCycle;\n');
        fprintf(write_file,'     file "poolDutyCycle.player"; //"constant_SP.player";\n');
        fprintf(write_file,'     loop 10000;\n');
        fprintf(write_file,'};\n\n');
        
        
    elseif use_flags.use_control == 1
                     
        fprintf(write_file,'object player {\n');
        fprintf(write_file,'     name controlOnOff;\n');
        fprintf(write_file,'     file "controlOnOff.player";\n');
        fprintf(write_file,'     loop 1000000;\n');
        fprintf(write_file,'};\n\n');
    ...
    if (use_flags.use_market ~= 0 && use_flags.use_market ~= 4)
        fprintf(write_file,'module market;\n\n');
    ...
    fprintf(write_file,'     name "%s";\n',tmy);
    fprintf(write_file,'     tmyfile ".\\\\Weather\\\\%s.tmy2";\n',tmy);
    ...
    fprintf(write_file,'          file ".\\\\Players\\\\V_A.player";\n');
    fprintf(write_file,'     };\n');
    fprintf(write_file,'     object player {\n');
    fprintf(write_file,'          property voltage_B;\n');
    fprintf(write_file,'          loop 100;\n');
    fprintf(write_file,'          file ".\\\\Players\\\\V_B.player";\n');
    fprintf(write_file,'     };\n');
    fprintf(write_file,'     object player {\n');
    fprintf(write_file,'          property voltage_C;\n');
    fprintf(write_file,'          loop 100;\n');
    fprintf(write_file,'          file ".\\\\Players\\\\V_C.player";\n');
    fprintf(write_file,'     };\n');
    ...
    for j=1:(nn-1)
        ...
        elseif (strcmp(char(glm_final{2}{j}),'triplex_node') ~= 0)
            fprintf(write_file,'%s %s %s %s %s %s %s %s\n',char(glm_final{1}{j}),char(glm_final{2}{j}),char(glm_final{3}{j}),char(glm_final{4}{j}),char(glm_final{5}{j}),char(glm_final{6}{j}),char(glm_final{7}{j}),char(glm_final{8}{j}));
            indxj = j; ispowernode = 'no';
            while (strcmp(char(glm_final{1}{indxj}),'}') ==0)
                if ~isempty(strfind(char(glm_final{2}{indxj}),'power'))
                    ispowernode = 'yes';
                end
                indxj = indxj+1;
            end
            if strcmp(ispowernode, 'no')
                fprintf(write_file,'      groupid Secondary_Triplex_Node;\n');
            end
        ...
        elseif (strcmp(char(glm_final{2}{j}),'triplex_meter') ~= 0)
            fprintf(write_file,'%s %s %s %s %s %s %s %s\n',char(glm_final{1}{j}),char(glm_final{2}{j}),char(glm_final{3}{j}),char(glm_final{4}{j}),char(glm_final{5}{j}),char(glm_final{6}{j}),char(glm_final{7}{j}),char(glm_final{8}{j}));
            fprintf(write_file,'      groupid Residential_Triplex_Meter;\n');
        ...
        elseif (strcmp(char(glm_final{2}{j}),'overhead_line') ~= 0)
            ...
            no_ohls = no_ohls + 1;
        elseif (strcmp(char(glm_final{2}{j}),'meter') ~= 0)
            fprintf(write_file,'%s %s %s %s %s %s %s %s\n',char(glm_final{1}{j}),char(glm_final{2}{j}),char(glm_final{3}{j}),char(glm_final{4}{j}),char(glm_final{5}{j}),char(glm_final{6}{j}),char(glm_final{7}{j}),char(glm_final{8}{j}));
            if (strcmp(char(glm_final{2}{j+1}),'name') ~= 0)
                meterName = char(glm_final{3}{j+1});
                loadName = strrep(meterName, 'meter', 'load');
                out = strfind([glm_final{3}{:}],loadName);
                if out > 0
        ...
        elseif (strcmp(char(glm_final{2}{j}),'load') ~= 0)
            if strcmp(char(glm_final{3}{j+1}),'R4-12-47-1_load_31;')
                qwerty = 1;
            end
            if (use_flags.use_commercial == 1) % Building Commercial
                ...
                while (strcmp(char(glm_final{1}{m}),'}') == 0)
                    ...
                    elseif (strcmp(char(glm_final{2}{m}),'name') ~= 0)
                        throw_away = char(glm_final{3}{m});% SJC edit parent_name = char(glm_final{3}{m});
                    ...
                    elseif (strcmp(char(glm_final{2}{m}),'parent') ~= 0)
                        parent_name = char(glm_final{3}{m}); % SJC edit from my_parent = char(glm_final{3}{m});  
                        %fprintf(write_file,'%s %s %s %s %s %s %s %s\n',char(glm_final{1}{m}),char(glm_final{2}{m}),char(glm_final{3}{m}),char(glm_final{4}{m}),char(glm_final{5}{m}),char(glm_final{6}{m}),char(glm_final{7}{m}),char(glm_final{8}{m}));        
                    end   
                ...
                % SJC edit fprintf(write_file,'}\n\n');
                no_loads = no_loads + 1;

                load_houses{4}{no_loads} = parent_name;
				if (load_A > 0 && load_B >0 && load_C >0)
                    no_houses = ceil((load_A +load_B + load_C)/ taxonomy_data.avg_commercial); % conservative rounding up 
                    no_houses_A = ceil(no_houses/3);
                    no_houses_B = ceil((no_houses-no_houses_A)/2);
                    no_houses_C = (no_houses-no_houses_A-no_houses_B);
					load_houses{5}{no_loads} = 'ABCN';
				elseif (load_A >0 && load_B >0)
                    no_houses = ceil((load_A +load_B)/ taxonomy_data.avg_commercial); % conservative rounding up 
                    no_houses_A = ceil(no_houses/2);
                    no_houses_B = no_houses-no_houses_A;  
					load_houses{5}{no_loads} = 'ABN';
				elseif (load_A >0 && load_C > 0)
                    no_houses = ceil((load_A +load_C)/ taxonomy_data.avg_commercial); % conservative rounding up 
                    no_houses_C = ceil(no_houses/2);
                    no_houses_A = no_houses-no_houses_C;  
					load_houses{5}{no_loads} = 'ACN';	
				elseif (load_B > 0 && load_C > 0)
                    no_houses = ceil((load_B +load_C)/ taxonomy_data.avg_commercial); % conservative rounding up 
                    no_houses_B = ceil(no_houses/2);
                    no_houses_C = no_houses-no_houses_B;  
					load_houses{5}{no_loads} = 'BCN';
				elseif (load_A > 0)
                    no_houses_A = ceil(load_A/ taxonomy_data.avg_commercial);
					load_houses{5}{no_loads} = 'AN';
				elseif (load_B > 0)
                    no_houses_B = ceil(load_B/ taxonomy_data.avg_commercial);
					load_houses{5}{no_loads} = 'BN';
				elseif (load_C > 0)
                    no_houses_C = ceil(load_C/ taxonomy_data.avg_commercial);
					load_houses{5}{no_loads} = 'CN';
                end 
                ...
                load_houses{9}{no_loads} = parent_name; %my_parent;
        ...
        elseif (strcmp(char(glm_final{2}{j}),'power_1') ~= 0 || strcmp(char(glm_final{2}{j}),'power_12') ~= 0)
            ...
            if (use_flags.use_homes == 1)
                ...
                if (no_of_houses > 0.5)
                    ...
                    if use_flags.use_streetlighting == 1
                    %if (tech_data.get_IEEE_stats == 0) SJCR edit
    ...
    if (use_flags.use_homes == 1 && total_houses ~= 0)
        ...
        for typeind=1:3
            ...
            cool_sp_save(:,typeind) = ceil(regional_data.cooling_setpoint{typeind}(:,1) * total_houses_by_type(typeind));
            heat_sp(:,typeind) = ceil(regional_data.heating_setpoint{typeind}(:,1) * total_houses_by_type(typeind));
            heat_sp_save(:,typeind) = ceil(regional_data.heating_setpoint{typeind}(:,1) * total_houses_by_type(typeind));
        end
        houseNum = 0;
        if (tech_data.get_IEEE_stats == 0)
            for jj=1:aS
                ...
                houseNum = houseNum+1;
                    ...
                    fprintf(write_file,'      groupid Single_House_Meter;\n');
                    ...
                    if (cool_type <= regional_data.perc_AC)
                        fprintf(write_file,'     groupid Residential_wAC;\n');
                    else
                        fprintf(write_file,'     groupid Residential_noAC;\n');
                    end
                    ...
                    if (use_flags.use_solar~=0 ||use_flags.use_solar_res~=0)
                        ...
                        solar_residential_array_name{jj}{kk} = ['solar_' 'house' num2str(kk) '_' parent2];
                    ...
                    numAirTemps = length(initData.airTemp);
                    if use_flags.use_setInitTemps ==1 && isfield(initData, 'airTemp') && numAirTemps>=houseNum
                        initAirTemp = initData.airTemp(houseNum);
                        initMassTemp = initData.massTemp(houseNum);
                        rand(1); % throw away a random # to keep rng in sync with past generated feeders
                    else initAirTemp = 68 + 4*rand(1);
                        initMassTemp = initAirTemp;
                    end
                    ...
                    elseif (floor_area*ceiling_height > 12000 ) % No resistive homes over with large volumes
                        ...
                        if use_flags.use_rngSetPoints ==1
                            cooling_set = ceil(regional_data.no_cool_sch*rand(altRng,1));  
                            heating_set = ceil(regional_data.no_heat_sch*rand(altRng,1));
                        end
                        ...
                        if use_flags.use_rngSetPoints ==0
                            ...
                            cool_bin_save = randi(no_cool_bins);
                            while (cool_sp_save(cool_bin_save,row_ti) < 1)
                                cool_bin_save = randi(no_cool_bins);
                            end
                            cool_sp_save(cool_bin_save,row_ti) = cool_sp_save(cool_bin_save,row_ti) - 1;
                            cool_bin = randi(altRng,no_cool_bins);
                            while (cool_sp(cool_bin,row_ti) < 1)
                                cool_bin = randi(altRng,no_cool_bins);
                            end
                            cool_sp(cool_bin,row_ti) = cool_sp(cool_bin,row_ti) - 1;
                        ...
                        heat_bin_save = randi(no_heat_bins);
                        if use_flags.use_rngSetPoints ==1
                            heat_bin = randi(altRng,no_heat_bins);
                        else heat_bin = heat_bin_save;
                        end
                        heat_count = 1;
                        ...
                        if use_flags.use_rngSetPoints ==0
                            ...
                            while (heat_sp_save(heat_bin_save,row_ti) < 1 || (heatsp(heat_bin_save,3) >= coolsp(cool_bin_save,4)))
                                heat_bin_save = randi(no_heat_bins);
                                % if we tried a few times, give up and take an extra
                                % draw from the lowest bin
                                if (heat_count > 20)
                            ...
                            heat_sp_save(heat_bin_save,row_ti) = heat_sp_save(heat_bin_save,row_ti) - 1;
                            heat_count = 1;
                            while (heat_sp(heat_bin,row_ti) < 1 || (heatsp(heat_bin,3) >= coolsp(cool_bin,4)))
                                heat_bin = randi(altRng,no_heat_bins);
                                % if we tried a few times, give up and take an extra
                        ...
                        if use_flags.use_rngSetPoints ==1 %override
                            cool_night = (coolsp(cool_bin,3) - coolsp(cool_bin,4))*rand(altRng,1) + coolsp(cool_bin,4) + 1;
                            heat_night = (heatsp(heat_bin,3) - heatsp(heat_bin,4))*rand(altRng,1) + heatsp(heat_bin,4) - 1;
                            cool_night_diff = coolsp(cool_bin,2) * 2 * rand(altRng,1);
                            heat_night_diff = heatsp(heat_bin,2) * 2 * rand(altRng,1);
                        end
                        
                    if use_flags.use_setCoolingStatusWithRng ==1
                        if cool_type <= regional_data.perc_AC
                            lowerBound = cool_night+cool_night_diff*2-1;
                            if initAirTemp > lowerBound
                                avail = avail+1;
                                if rand(altRng,1) < initData.coolOnRatio
                                    fprintf(write_file,'     system_mode COOL;\n');
                                    fprintf(write_file,'     last_system_mode COOL;\n');
                                else fprintf(write_file,'     system_mode OFF;\n');
                                    fprintf(write_file,'     last_system_mode OFF;\n');
                                end
                            else fprintf(write_file,'     system_mode OFF;\n');
                                fprintf(write_file,'     last_system_mode OFF;\n');
                            end
                        end
                    elseif use_flags.use_setHvacSystemMode ==1
                        fprintf(write_file,'     system_mode %s;\n', initData.sysMode{houseNum});
                        fprintf(write_file,'     last_system_mode %s;\n',initData.sysMode{houseNum});
                    end
                    % If we have markets, put in a controller (maybe)
                    if (use_flags.use_market == 0 && use_flags.use_control == 0)
                    ...
                    elseif (use_flags.use_market == 0 && use_flags.use_control == 1) % normal and probonoff control
                        fprintf(write_file,'     cooling_setpoint cooling%d*%.2f+%.2f;\n',cooling_set,cool_night_diff,cool_night);
                        fprintf(write_file,'     heating_setpoint heating%d*%.2f+%.2f;\n',heating_set,heat_night_diff,heat_night);
                                                
                        fprintf(write_file,'     object passive_controller {\n');
                        fprintf(write_file,'          control_mode PROBABILITY_ONOFF;\n');
                        fprintf(write_file,'          distribution_type UNIFORM;\n');
                        fprintf(write_file,'          period %d;\n', tech_data.meas_interval);
                        fprintf(write_file,'          state_property override;\n');
                        fprintf(write_file,'          observation_object controlOnOff;\n');
                        fprintf(write_file,'          observation_property value;\n');
                        fprintf(write_file,'          comfort_level 1;\n');
                        fprintf(write_file,'     };\n');
                        
                    elseif (use_flags.use_market == 0 && use_flags.use_control == 2)
                        fprintf(write_file,'     cooling_setpoint setPointAC.value*%.2f;\n',cool_night);
                        fprintf(write_file,'     heating_setpoint heating%d*%.2f+%.2f;\n',heating_set,heat_night_diff,heat_night);
                        
                    elseif (use_flags.use_market == 3 && use_flags.use_control == 0) % dlc
                        fprintf(write_file,'     cooling_setpoint cooling%d*%.2f+%.2f;\n',cooling_set,cool_night_diff,cool_night);
                        fprintf(write_file,'     heating_setpoint heating%d*%.2f+%.2f;\n',heating_set,heat_night_diff,heat_night);
                    ...
                    if  use_flags.use_addBaseLoad == 1
                        if rand(altRng,1) < tech_data.pctExtraBaseload
                            fprintf(write_file,'     object ZIPload {\n');
                            fprintf(write_file,'           // Additional baseload \n');           
                            fprintf(write_file,'           base_power %f;\n',tech_data.baseload);
                            fprintf(write_file,'           heatgain_fraction 0;\n');
                            fprintf(write_file,'           power_pf 1;\n');
                            fprintf(write_file,'           current_pf 1;\n');
                            fprintf(write_file,'           impedance_pf 1;\n');
                            fprintf(write_file,'           impedance_fraction 0;\n');
                            fprintf(write_file,'           current_fraction 0;\n');
                            fprintf(write_file,'           power_fraction 1;\n');
                            fprintf(write_file,'     };\n');
                        end
                    end
                    ...
                    if (pool_pump_perc < 2*regional_data.perc_poolpumps && no_pool_pumps >= 1 && row_ti == 1)
                        ...
                        else fprintf(write_file,'           duty_cycle poolDutyCycle.value*%.2f + %.2f;\n',(1-pp_dutycycle),pp_dutycycle);
                        end
                        fprintf(write_file,'           phase %.2f;\n',pp_init_phase);
                        fprintf(write_file,'           period %.2f;\n',pp_period);
                        fprintf(write_file,'           heatgain_fraction 0.0;\n');
                        fprintf(write_file,'           power_pf %.3f;\n',tech_data.p_pf_pool);  %SJCR edit 1/12/2018
                        fprintf(write_file,'           current_pf %.3f;\n',tech_data.i_pf_pool); %
                        fprintf(write_file,'           impedance_pf %.3f;\n',tech_data.z_pf_pool); %
                        fprintf(write_file,'           impedance_fraction %f;\n',tech_data.zfrac);
                        fprintf(write_file,'           current_fraction %f;\n',tech_data.ifrac);
                    ...
                    if (heat_type > (1 - regional_data.wh_electric) && tech_data.use_wh == 1)
                        ...
                        if (use_flags.use_control == 0 || use_flags.use_control == 1)
                            fprintf(write_file,'          tank_setpoint %.1f;\n',tank_set);
                        elseif use_flags.use_control == 2
                            fprintf(write_file,'          tank_setpoint setPointWaterHeater.value*%.1f;\n',tank_set);
                        end
    ...
    if (no_loads ~= 0 && use_flags.use_commercial == 1)
        ...
        for iii = 1:r2
            ...
            has_phase_C = 0;
            ph = '';
			if (load_houses{1,6}{iii} ~= 0)
                has_phase_A = 1;
                ph = strcat(ph,'A');
            end
            if (load_houses{1,7}{iii} ~= 0)
                has_phase_B = 1;
                ph = strcat(ph,'B');
            end
            if (load_houses{1,8}{iii} ~= 0)
                has_phase_C = 1;
                ph = strcat(ph,'C');
            end
            %{
            ...
            total_comm_houses_remainder = total_comm_houses;
            % Office building - must have all three phases and enough load for
            % 15 zones
            if (total_comm_houses > 15 && no_of_phases == 3)
                no_of_offices = floor(total_comm_houses / 15);  % SJCR edit 1/27/2018
                total_comm_houses_remainder = total_comm_houses - no_of_offices*15;
                ...
                fprintf(write_file,'object transformer_configuration {\n');
                fprintf(write_file,'     name CTTF_config_A_office_%s;\n',my_name);  % SJCR edit 1/27/2018
                ...
                fprintf(write_file,'object transformer_configuration {\n');
                fprintf(write_file,'     name CTTF_config_B_office_%s;\n',my_name);  % SJCR edit 1/27/2018
                fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                fprintf(write_file,'     install_type POLETOP;\n');
                fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                fprintf(write_file,'     shunt_impedance 10000+10000j;\n');
                ...
                fprintf(write_file,'object transformer_configuration {\n');
                fprintf(write_file,'     name CTTF_config_C_office_%s;\n',my_name);  % SJCR edit 1/27/2018
                fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                fprintf(write_file,'     install_type POLETOP;\n');
                fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                fprintf(write_file,'     shunt_impedance 10000+10000j;\n');
                ...
                for jjj = 1:no_of_offices
                    ...
                    if (tech_data.get_IEEE_stats == 0)
                        for phind = 1:3                  
                            fprintf(write_file,'object transformer {\n');
                            fprintf(write_file,'     name %s_CTTF_%s_office_%.0f;\n',my_name,my_phases{phind},jjj); % SJCR edit 1/27/2018
                            fprintf(write_file,'     phases %sS;\n',my_phases{phind});
                            fprintf(write_file,'     from %s_office_meter%.0f;\n',my_name,jjj);
                            fprintf(write_file,'     to %s_tm_office_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     groupid %s;\n','Distribution_Trans');
                            fprintf(write_file,'     configuration CTTF_config_%s_office_%s;\n',my_phases{phind},my_name);
                            fprintf(write_file,'}\n\n');


                            fprintf(write_file,'object triplex_meter {\n');
                            fprintf(write_file,'     name %s_tm_office_%s_%.0f;\n',my_name,my_phases{phind},jjj); % SJCR edit 1/27/2018
                            fprintf(write_file,'     groupid Commercial_Triplex_Meter;\n');
                            fprintf(write_file,'     phases %sS;\n',my_phases{phind});
                            fprintf(write_file,'     nominal_voltage 120;\n');
                            fprintf(write_file,'}\n\n');
                            ...
                            for zoneind=1:5
                                ...
                                fprintf(write_file,'     parent %s_tm_office_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                ...
                                if (use_flags.use_solar~=0 || use_flags.use_solar_com ~=0)
                                    ...
                                    solar_office_array_name{iii}{jjj}{phind}{zoneind} = ['solar_office' my_name '_' my_phases{phind} num2str(jjj) '_zone' num2str(zoneind)];
                                ...
                                elseif use_flags.use_control == 2
                                    fprintf(write_file,'     cooling_setpoint setPointAC.value*75.0;\n');
                                else
                                ...
                                %{
                                if use_flags.use_setCoolingStatusWithRng ==1 % replicate this for other commercial buildings
                                    lowerBound = 75 - 1; 
                                    if initAirTemp > lowerBound
                                        if rand(altRng,1) < initData.coolOnRatio
                                            fprintf(write_file,'     system_mode COOL;\n');
                                ...
                                %}
                                if use_flags.use_setHvacSystemMode ==1
                                    fprintf(write_file,'     system_mode %s;\n', initData.sysMode{houseNum});
                                    fprintf(write_file,'     last_system_mode %s;\n',initData.sysMode{houseNum});
                                end
                                ...
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name lights_office_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                ...
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name plugs_office_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name wh_office_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                ...
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name ext_office_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 0.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name occ_office_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction 0.0;\n');
                                fprintf(write_file,'          impedance_fraction 0.0;\n');
            ...
            if (total_comm_houses_remainder > 6 && no_of_phases >= 2)
                no_of_bigboxes = floor(total_comm_houses_remainder / 6);  % SJCR edit from "round" 1/27/18
                total_comm_houses_remainder = total_comm_houses_remainder - no_of_bigboxes*6;  % edit cont.
                ph3_meter = ph3_meter + 1;
                ...
                if (has_phase_A == 1)
                    fprintf(write_file,'object transformer_configuration {\n');
                    fprintf(write_file,'     name CTTF_config_A_box_%s;\n',my_name); % SJCR edit 1/27/2018
                    fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                    fprintf(write_file,'     install_type POLETOP;\n');
                    fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                    fprintf(write_file,'     shunt_impedance 10000+10000j;\n');
                ...
                if (has_phase_B == 1)
                    fprintf(write_file,'object transformer_configuration {\n');
                    fprintf(write_file,'     name CTTF_config_B_box_%s;\n',my_name);
                    fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                    fprintf(write_file,'     install_type POLETOP;\n');
                    fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                    fprintf(write_file,'     shunt_impedance 10000+10000j;\n');
                ...
                if (has_phase_C == 1)
                    fprintf(write_file,'object transformer_configuration {\n');
                    fprintf(write_file,'     name CTTF_config_C_box_%s;\n',my_name);
                    fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                    fprintf(write_file,'     install_type POLETOP;\n');
                    fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                    fprintf(write_file,'     shunt_impedance 10000+10000j;\n');
                ...
                for jjj = 1:no_of_bigboxes
                    ...
                    if (tech_data.get_IEEE_stats == 0)
                        for phind=1:3
                            ...
                            fprintf(write_file,'object transformer {\n');
                            fprintf(write_file,'     name %s_CTTF_%s_box_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     phases %sS;\n',my_phases{phind});
                            fprintf(write_file,'     from %s_bigbox_meter%.0f;\n',my_name,jjj);
                            fprintf(write_file,'     to %s_tm_box_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     groupid %s;\n','Distribution_Trans');
                            fprintf(write_file,'     configuration CTTF_config_%s_box_%s;\n',my_phases{phind},my_name);
                            fprintf(write_file,'}\n\n');


                            fprintf(write_file,'object triplex_meter {\n');
                            fprintf(write_file,'     name %s_tm_box_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     groupid Commercial_Triplex_Meter;\n');
                            fprintf(write_file,'     phases %sS;\n',my_phases{phind});
                            fprintf(write_file,'     nominal_voltage 120;\n');
                            fprintf(write_file,'}\n\n');
                            ...
                            for zoneind=1:zones_per_phase
                                ...
                                fprintf(write_file,'     groupid Commercial;\n');
                                fprintf(write_file,'     schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'     parent %s_tm_box_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'     floor_area %.0f;\n',floor_area);
                                fprintf(write_file,'     design_internal_gains %.0f;\n',int_gains*floor_area*3.413);
                                fprintf(write_file,'     number_of_doors %.0f;\n',no_of_doors);
                                fprintf(write_file,'     aspect_ratio %.2f;\n',aspect_ratio);
                                ...
                                if (use_flags.use_solar~=0 ||use_flags.use_solar_com ~=0)
                                    ...
                                    solar_bigbox_array_name{iii}{jjj}{phind}{zoneind} = ['solar_bigbox' my_name '_' my_phases{phind} num2str(jjj) '_zone' num2str(zoneind)];
                                ...
                                elseif use_flags.use_control == 2
                                    fprintf(write_file,'     cooling_setpoint setPointAC.value*75.0;\n');
                                else fprintf(write_file,'     cooling_setpoint bigbox_cooling;\n');
                                ...
                                fprintf(write_file,'     // Lights\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name lights_box_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     // Plugs\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name plugs_box_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     // Exterior Lighting\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name ext_box_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 0.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     // Occupancy\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name occ_box_%s_%s_%.0f_zone%.0f;\n',my_name,my_phases{phind},jjj,zoneind);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction 0.0;\n');
                                fprintf(write_file,'          impedance_fraction 0.0;\n');
            ...
            % Strip mall
            if (total_comm_houses_remainder > 0)
                no_of_strip = total_comm_houses_remainder;
                ph1_meter = ph1_meter + 1;
                
                my_name = strrep(load_houses{1,4}{iii},';','');

                strip_per_phase = round(no_of_strip / no_of_phases); % why even do this by rounding... can't we place the correct number exactly? 
                if strip_per_phase> 0
                    if (has_phase_A == 1)
                        fprintf(write_file,'object transformer_configuration {\n');
                        fprintf(write_file,'     name CTTF_config_A_strip_%s;\n',my_name);
                        fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                        fprintf(write_file,'     install_type POLETOP;\n');
                        fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                    ...
                    if (has_phase_B == 1)
                        fprintf(write_file,'object transformer_configuration {\n');
                        fprintf(write_file,'     name CTTF_config_B_strip_%s;\n',my_name);
                        fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                        fprintf(write_file,'     install_type POLETOP;\n');
                        fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                        fprintf(write_file,'     shunt_impedance 100000+100000j;\n');
                    ...
                    if (has_phase_C == 1)
                        fprintf(write_file,'object transformer_configuration {\n');
                        fprintf(write_file,'     name CTTF_config_C_strip_%s;\n',my_name);
                        fprintf(write_file,'     connect_type SINGLE_PHASE_CENTER_TAPPED;\n');
                        fprintf(write_file,'     install_type POLETOP;\n');
                        fprintf(write_file,'     impedance 0.00033+0.0022j;\n');
                        fprintf(write_file,'     shunt_impedance 100000+100000j;\n');
                    ...
                    fprintf(write_file,'object overhead_line {\n');
                    fprintf(write_file,'     from %s;\n',my_name);
                    fprintf(write_file,'     to %s_strip_meter;\n',my_name);
                    fprintf(write_file,'     phases %s;\n',load_houses{1,5}{iii});
                    fprintf(write_file,'     length 50ft;\n');
                    fprintf(write_file,'     configuration line_configuration_comm%s;\n',ph);
                    fprintf(write_file,'};\n\n');
                    ...
                    fprintf(write_file,'object meter {\n');            
                    fprintf(write_file,'     phases %s;\n',load_houses{1,5}{iii});
                    fprintf(write_file,'     name %s_strip_meter;\n',my_name);
                    fprintf(write_file,'     nominal_voltage %f;\n',taxonomy_data.nom_volt2);
                    fprintf(write_file,'}\n\n');
                    ...
                    for phind=1:3
                        ...
                        for jjj = 1:strip_per_phase
                            ...
                            fprintf(write_file,'object transformer {\n');
                            fprintf(write_file,'     name %s_CTTF_%s_strip_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     phases %sS;\n',my_phases{phind});
                            fprintf(write_file,'     from %s_strip_meter;\n',my_name);
                            fprintf(write_file,'     to %s_tm_strip_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     groupid %s;\n','Distribution_Trans');
                            fprintf(write_file,'     configuration CTTF_config_%s_strip_%s;\n',my_phases{phind},my_name);
                            fprintf(write_file,'}\n\n');

                            fprintf(write_file,'object triplex_meter {\n');
                            fprintf(write_file,'     name %s_tm_strip_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                            fprintf(write_file,'     phases %sS;\n',my_phases{phind});
                            fprintf(write_file,'     groupid Commercial_Triplex_Meter;\n');
                            ...
                            if (tech_data.get_IEEE_stats == 0)
                                ...
                                fprintf(write_file,'     groupid Commercial;\n');
                                fprintf(write_file,'     schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'     parent %s_tm_strip_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'     floor_area %.0f;\n',floor_area);
                                fprintf(write_file,'     design_internal_gains %.0f;\n',int_gains*floor_area*3.413);
                                fprintf(write_file,'     number_of_doors %.0f;\n',no_of_doors);
                                fprintf(write_file,'     aspect_ratio %.2f;\n',aspect_ratio);
                                ...
                                if (use_flags.use_solar~=0 || use_flags.use_solar_com ~=0)
                                    ...
                                    solar_stripmall_array_name{iii}{jjj}{phind} = [my_name '_tmsolar_' my_phases{phind} '_' num2str(jjj)];
                                    solar_stripmall_array_parent{iii}{jjj}{phind} = [my_name '_tm_' my_phases{phind} '_' num2str(jjj)];
                                ...
                                elseif  use_flags.use_control ==2
                                    fprintf(write_file,'     cooling_setpoint setPointAC.value*75.0;\n');
                                else fprintf(write_file,'     cooling_setpoint stripmall_cooling;\n');
                                    if (use_flags.use_market == 3)    
                                        fprintf(write_file,'          object passive_controller {\n');
                                        fprintf(write_file,'               control_mode DIRECT_LOAD_CONTROL;\n');
                                        fprintf(write_file,'               dlc_mode CYCLING;\n');
                                ...
                                fprintf(write_file,'     // Lights\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name lights_strip_%s_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     // Plugs\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name plugs_strip_%s_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     // Gas Waterheater\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name wh_strip_%s_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction 0.0;\n');
                                fprintf(write_file,'          impedance_fraction 0.0;\n');
                                ...
                                fprintf(write_file,'     // Exterior Lighting\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name ext_strip_%s_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 0.0;\n');
                                fprintf(write_file,'          power_fraction %.2f;\n',tech_data.c_pfrac);
                                fprintf(write_file,'          impedance_fraction %.2f;\n',tech_data.c_zfrac);
                                ...
                                fprintf(write_file,'     // Occupancy\n');
                                fprintf(write_file,'     object ZIPload {\n');
                                fprintf(write_file,'          name occ_strip_%s_%s_%.0f;\n',my_name,my_phases{phind},jjj);
                                fprintf(write_file,'          schedule_skew %.0f;\n',skew_value);
                                fprintf(write_file,'          heatgain_fraction 1.0;\n');
                                fprintf(write_file,'          power_fraction 0.0;\n');
                                fprintf(write_file,'          impedance_fraction 0.0;\n');
    ...
    if (use_flags.use_solar_res~=0 && use_flags.use_solar==0)
        penetration_residential = regional_data.solar_penetration(region);
    end
    if (use_flags.use_solar~=0  ||use_flags.use_solar_com ~=0)
        ...
        elseif use_flags.use_solar~=0  % if using combination of commercial and residential
             penetration_stripmall = regional_data.solar_penetration(region)/4;
             penetration_bigbox = regional_data.solar_penetration(region)/4;
             penetration_office = regional_data.solar_penetration(region)/4;
             penetration_residential = regional_data.solar_penetration(region)/4;
        end
                            ...
                            for jjjj=1:length(solar_office_array{jj}{jjj})
                                for jjjjj=1:length(solar_office_array{jj}{jjj}{jjjj})
                                    if (~isempty(solar_office_array{jj}{jjj}{jjjj}{jjjjj}))
                                            ...
                                            name = solar_office_array_name{jj}{jjj}{jjjj}{jjjjj}; 
                            ...
                            for jjjj=1:length(solar_bigbox_array{jj}{jjj})
                                for jjjjj=1:length(solar_bigbox_array{jj}{jjj}{jjjj})
                                    if (~isempty(solar_bigbox_array{jj}{jjj}{jjjj}{jjjjj}))
                                            ...
                                            name = solar_bigbox_array_name{jj}{jjj}{jjjj}{jjjjj};
                        ...
                        for jjj=1:length(solar_stripmall_array{jj})
                            for jjjj=1:length(solar_stripmall_array{jj}{jjj})
                                if (~isempty(solar_stripmall_array{jj}{jjj}{jjjj}))
                                    ...
                                    name = solar_stripmall_array_name{jj}{jjj}{jjjj};
           ...
           residential_count=1;
            ...
            for jj=1:total_no_residential_length
                ...
                if (~isempty(solar_residential_array{jj}))            
                    ...
                    if (use_flags.use_solar ~= 0 ||use_flags.use_solar_res ~= 0)
                    number_solar_house =  (taxonomy_data.emissions_peak * penetration_residential)/(tech_data.solar_averagepower);
                    number_solar_house_perm = number_solar_house; % Added by SJCR on 1/8/2018
                    end
                                                                                  
                    for jjj=1:length(solar_residential_array{jj})
                        
                        if (~isempty(solar_residential_array{jj}{jjj}))
                            % Added by SJCR on 1/8/2018 {
                            phase1 = solar_residential_array_phase{jj}{jjj}; 
                            number_solar_house = number_solar_house_perm;
                            if strcmp(phase1,'AS;')
                                number_solar_house = number_solar_house*1.5;
                            elseif strcmp(phase1,'BS;')
                                number_solar_house = number_solar_house*0.5;
                            elseif strcmp(phase1,'CS;')
                                number_solar_house = number_solar_house*0.5;
                            end
                            %}
                            if  (residential_penetration(residential_count) < (number_solar_house/total_no_residential))
                                
                                name = solar_residential_array_name{jj}{jjj}; 
                                ...
                                fprintf(write_file,'      parent %s\n',parent);
                                fprintf(write_file,'      nominal_voltage 120;\n');
                                fprintf(write_file,'      groupid Residential_tm_solar_%s\n\n',phase1); % Modified by SJCR on 1/8/2018
...
%     end
    ...
    fprintf(write_file,'object recorder {\n');
    fprintf(write_file,'     file %s_subStationTransformerPower.csv;\n',file4);
    ...
    if use_flags.use_control == 1
        ...
        fprintf(write_file,'     name hvacCollectorRes;\n');
        fprintf(write_file,'     file %s_hvacDataRes.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Residential_wAC";\n');
        fprintf(write_file,'     interval %d;\n',tech_data.meas_interval);
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        fprintf(write_file,'     property avg(cooling_setpoint),avg(air_temperature), avg(mass_temperature),sum(is_AUX_on),sum(is_HEAT_on),sum(is_COOL_on),sum(hvac_power.real);\n');
        fprintf(write_file,'}\n\n');
        
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name hvacCollectorComm;\n');
        fprintf(write_file,'     file %s_hvacDataComm.csv;\n',file4);
        ...
        %%%%%NEEDED FOR MATLAB CONTROL LINK%%%%%%%%%%%%%%%%%%%%%%
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name hvacCollector;\n');
        fprintf(write_file,'     file %s_hvacPowerRes.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Residential_wAC";\n');
        fprintf(write_file,'     interval %d;\n',tech_data.meas_interval);
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        fprintf(write_file,'     property sum(hvac_power.real);\n');
        fprintf(write_file,'}\n\n');
        
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name hvacOnAvailability;\n');
        fprintf(write_file,'     file %s_hvacOnAvailability.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Residential_wAC";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 47980;\n');
        fprintf(write_file,'     property sum(hvac_onAndAvailable);\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name hvacOffAvailability;\n');
        fprintf(write_file,'     file %s_hvacOffAvailability.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Residential_wAC";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 47980;\n');
        fprintf(write_file,'     property sum(hvac_offAndAvailable);\n');
        fprintf(write_file,'     }\n\n');
        
             
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name acCounter;\n');
        fprintf(write_file,'     file %s_numResidentialAcOn.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Residential_wAC";\n');
        fprintf(write_file,'     interval %d;\n',tech_data.meas_interval);
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        ...
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name hvacOnAvailability_comm;\n');
        fprintf(write_file,'     file %s_hvacOnAvailability_comm.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Commercial";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property sum(hvac_onAndAvailable);\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     name hvacOffAvailability_comm;\n');
        fprintf(write_file,'     file %s_hvacOffAvailability_comm.csv;\n',file4);
        fprintf(write_file,'     group "class=house AND groupid=Commercial";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property sum(hvac_offAndAvailable);\n');
        ...
        fprintf(write_file,'     name houseCoolOn;\n');
        fprintf(write_file,'     file %s_houseCoolOn.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Residential_wAC";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property is_COOL_on;\n');
        fprintf(write_file,'     }\n\n');
                
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name hvacSystemMode;\n');
        fprintf(write_file,'     file %s_hvacSystemMode.csv;\n',file4);
        fprintf(write_file,'     group "class=house";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property system_mode;\n');
        fprintf(write_file,'     }\n\n');
        
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name hvacPowers_real;\n');
        fprintf(write_file,'     file %s_hvacPowers_real.csv;\n',file4);
        fprintf(write_file,'     group "class=house";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property hvac_power;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name hvacPowers_imag;\n');
        fprintf(write_file,'     file %s_hvacPowers_imag.csv;\n',file4);
        fprintf(write_file,'     group "class=house";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property hvac_power;\n');
        fprintf(write_file,'     complex_part IMAG;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name indoorAirTemps;\n');
        fprintf(write_file,'     file %s_indoorAirTempAllHouses.csv;\n',file4);
        fprintf(write_file,'     group "class=house";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property air_temperature;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name indoorMassTemps;\n');
        fprintf(write_file,'     file %s_indoorMassTempAllHouses.csv;\n',file4);
        fprintf(write_file,'     group "class=house";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property mass_temperature;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object recorder {\n');
        fprintf(write_file,'     parent house1_%s_tm_1;\n',file5);
        fprintf(write_file,'     name outdoorTemp;\n');
        fprintf(write_file,'     file %s_outdoorTemp.csv;\n',file4);
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property outdoor_temperature;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltage1_tnRes_real;\n');
        fprintf(write_file,'     file %s_voltage1_tnRes_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_1;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltage1_tnRes_imag;\n');
        fprintf(write_file,'     file %s_voltage1_tnRes_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_1;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltage2_tnRes_real;\n');
        fprintf(write_file,'     file %s_voltage2_tnRes_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_2;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltage2_tnRes_imag;\n');
        fprintf(write_file,'     file %s_voltage2_tnRes_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_2;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageN_tnRes_real;\n');
        fprintf(write_file,'     file %s_voltageN_tnRes_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_N;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageN_tnRes_imag;\n');
        fprintf(write_file,'     file %s_voltageN_tnRes_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_N;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltage12_tnRes_real;\n');
        fprintf(write_file,'     file %s_voltage12_tnRes_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_12;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltage12_tnRes_imag;\n');
        fprintf(write_file,'     file %s_voltage12_tnRes_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Secondary_Triplex_Node";\n');
        fprintf(write_file,'     property voltage_12;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageA_tmCom_real;\n');
        fprintf(write_file,'     file %s_voltageA_tmCom_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Commercial_Meter";\n');
        fprintf(write_file,'     property measured_voltage_A;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageA_tmCom_imag;\n');
        fprintf(write_file,'     file %s_voltageA_tmCom_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Commercial_Meter";\n');
        fprintf(write_file,'     property measured_voltage_A;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageB_tmCom_real;\n');
        fprintf(write_file,'     file %s_voltageB_tmCom_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Commercial_Meter";\n');
        fprintf(write_file,'     property measured_voltage_B;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageB_tmCom_imag;\n');
        fprintf(write_file,'     file %s_voltageB_tmCom_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Commercial_Meter";\n');
        fprintf(write_file,'     property measured_voltage_B;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageC_tmCom_real;\n');
        fprintf(write_file,'     file %s_voltageC_tmCom_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Commercial_Meter";\n');
        fprintf(write_file,'     property measured_voltage_C;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageC_tmCom_imag;\n');
        fprintf(write_file,'     file %s_voltageC_tmCom_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Commercial_Meter";\n');
        fprintf(write_file,'     property measured_voltage_C;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageA_node_real;\n');
        fprintf(write_file,'     file %s_voltageA_node_real.csv;\n',file4);
        fprintf(write_file,'     group "class=node";\n');
        fprintf(write_file,'     property voltage_A;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageA_node_imag;\n');
        fprintf(write_file,'     file %s_voltageA_node_imag.csv;\n',file4);
        fprintf(write_file,'     group "class=node";\n');
        fprintf(write_file,'     property voltage_A;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageB_node_real;\n');
        fprintf(write_file,'     file %s_voltageB_node_real.csv;\n',file4);
        fprintf(write_file,'     group "class=node";\n');
        fprintf(write_file,'     property voltage_B;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageB_node_imag;\n');
        fprintf(write_file,'     file %s_voltageB_node_imag.csv;\n',file4);
        fprintf(write_file,'     group "class=node";\n');
        fprintf(write_file,'     property voltage_B;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageC_node_real;\n');
        fprintf(write_file,'     file %s_voltageC_node_real.csv;\n',file4);
        fprintf(write_file,'     group "class=node";\n');
        fprintf(write_file,'     property voltage_C;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name voltageC_node_imag;\n');
        fprintf(write_file,'     file %s_voltageC_node_imag.csv;\n',file4);
        fprintf(write_file,'     group "class=node";\n');
        fprintf(write_file,'     property voltage_C;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name lineCurrentA_real;\n');
        fprintf(write_file,'     file %s_lineCurrentA_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Line";\n');
        fprintf(write_file,'     property current_out_A;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name lineCurrentA_imag;\n');
        fprintf(write_file,'     file %s_lineCurrentA_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Line";\n');
        fprintf(write_file,'     property current_out_A;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name lineCurrentB_real;\n');
        fprintf(write_file,'     file %s_lineCurrentB_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Line";\n');
        fprintf(write_file,'     property current_out_B;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name lineCurrentB_imag;\n');
        fprintf(write_file,'     file %s_lineCurrentB_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Line";\n');
        fprintf(write_file,'     property current_out_B;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name lineCurrentC_real;\n');
        fprintf(write_file,'     file %s_lineCurrentC_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Line";\n');
        fprintf(write_file,'     property current_out_C;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name lineCurrentC_imag;\n');
        fprintf(write_file,'     file %s_lineCurrentC_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Line";\n');
        fprintf(write_file,'     property current_out_C;\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name triplexMeterPower_imag;\n');
        fprintf(write_file,'     file %s_triplexMeterPower_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Residential_Triplex_Meter";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property measured_power;\n');
        fprintf(write_file,'     complex_part IMAG;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name triplexMeterPower_real;\n');
        fprintf(write_file,'     file %s_triplexMeterPower_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Residential_Triplex_Meter";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property measured_power;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name xfmrPower_real;\n');
        fprintf(write_file,'     file %s_xfmrPower_real.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Trans";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property power_out;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name xfmrPower_imag;\n');
        fprintf(write_file,'     file %s_xfmrPower_imag.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Trans";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property power_out;\n');
        ...
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name xfmrPowLoss;\n');
        fprintf(write_file,'     file %s_xfmrPowLoss.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Trans";\n');
        fprintf(write_file,'     interval %d;\n',tech_data.meas_interval);
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        fprintf(write_file,'     property power_losses;\n');
        fprintf(write_file,'     complex_part REAL;\n');
        fprintf(write_file,'}\n\n');
    ...
    if use_flags.use_xfmrThermalModel == 1    
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name xfmrLossOfLife;\n');
        fprintf(write_file,'     file %s_xfmrLossOfLife.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Trans";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property percent_loss_of_life;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name xfmrCoilTemp;\n');
        fprintf(write_file,'     file %s_xfmrCoilTemp.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Trans";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property winding_hot_spot_temperature;\n');
        fprintf(write_file,'     }\n\n');
        
        fprintf(write_file,'object group_recorder {\n');
        fprintf(write_file,'     name xfmrOilTemp;\n');
        fprintf(write_file,'     file %s_xfmrOilTemp.csv;\n',file4);
        fprintf(write_file,'     group "groupid=Distribution_Trans";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit 1000000;\n');
        fprintf(write_file,'     property top_oil_hot_spot_temperature;\n');
        fprintf(write_file,'     }\n\n');
    ...
    % Modified by SJCR 1/8/2018           
    if (countSOLAR > 0)
        % Solar collector for residential
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     file %s_Solar_A.csv;\n',tech_file);
        fprintf(write_file,'     group "class=triplex_meter AND groupid=Residential_tm_solar_AS";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        fprintf(write_file,'     property sum(measured_power.real),sum(measured_power.imag);\n');
        fprintf(write_file,'}\n\n');
        
        % Solar collector for residential
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     file %s_Solar_B.csv;\n',tech_file);
        fprintf(write_file,'     group "class=triplex_meter AND groupid=Residential_tm_solar_BS";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        fprintf(write_file,'     property sum(measured_power.real),sum(measured_power.imag);\n');
        fprintf(write_file,'}\n\n');
        ...
        % Solar collector for residential
        fprintf(write_file,'object collector {\n');
        fprintf(write_file,'     file %s_Solar_C.csv;\n',tech_file);
        fprintf(write_file,'     group "class=triplex_meter AND groupid=Residential_tm_solar_CS";\n');
        fprintf(write_file,'     interval 2;\n');
        fprintf(write_file,'     limit %d;\n',tech_data.meas_limit);
        fprintf(write_file,'     property sum(measured_power.real),sum(measured_power.imag);\n');
        fprintf(write_file,'}\n\n');
    ...
    if (tech_data.measure_market == 1 && use_flags.use_market ~= 0 && use_flags.use_market ~=4)
        fprintf(write_file,'object recorder {\n');
        fprintf(write_file,'     parent %s;\n',tech_data.market_info{1});
    ...
    if (tech_data.collect_setpoints == 1)
        ...
        %}
         fprintf(write_file, 'object collector {\n');
         fprintf(write_file,'group "class=waterheater";\n'); 
         fprintf(write_file,'property avg(tank_setpoint),sum(is_waterheater_on);\n');
         fprintf(write_file,'interval 60;\n');
         fprintf(write_file,'limit 702720;\n');
         fprintf(write_file,'file %s_waterheaterSetpoints.csv;\n',file4);
         fprintf(write_file,'}\n\n');
         
         fprintf(write_file, 'object collector {\n');
         fprintf(write_file,'group "class=house";\n'); 
         fprintf(write_file,'property avg(cooling_setpoint),avg(air_temperature),sum(is_AUX_on),sum(is_HEAT_on),sum(is_COOL_on),sum(hvac_power.real);\n');
         fprintf(write_file,'interval 60;\n');
         fprintf(write_file,'limit 702720;\n');
         fprintf(write_file,'file %s_hvacSetpoints.csv;\n',file4);
         fprintf(write_file,'}\n\n');
    end
    ...
    if (tech_data.measure_loads ~= 0)
        ...
        if (total_houses > 0)
            fprintf(write_file,'//object collector {\n');
            fprintf(write_file,'//     group "class=triplex_meter AND groupid=Residential_Triplex_Meter";\n'); %SJC edit
    ...
    if use_flags.use_violationRecorder == 1
        fprintf(write_file,'object violation_recorder {\n');
        fprintf(write_file,'     violation_flag ALLVIOLATIONS;\n');
        fprintf(write_file,'     violation_delay 0; // One day delay = 86400 \n'); 
        fprintf(write_file,'     strict false;\n');
        fprintf(write_file,'     echo false;\n');
        fprintf(write_file,'     interval %d;\n', tech_data.meas_interval);
        fprintf(write_file,'     summary %s_violationSummary.csv;\n',file4);
        fprintf(write_file,'     file %s_violationLog.csv;\n\n',file4);

        fprintf(write_file,'     // Violation 1 - Exceeding device thermal limit\n');
        fprintf(write_file,'     line_thermal_limit_lower 0;\n');
        fprintf(write_file,'     line_thermal_limit_upper 1;\n');
        fprintf(write_file,'     xfrmr_thermal_limit_lower 0;\n');
        fprintf(write_file,'     xfrmr_thermal_limit_upper 2;\n\n');

        fprintf(write_file,'     // Violation 2 - Instantaneous voltage of node over X pu or under Y pu;\n');
        fprintf(write_file,'     node_instantaneous_voltage_limit_lower 0.9;\n');
        fprintf(write_file,'     node_instantaneous_voltage_limit_upper 1.083;\n\n');

        fprintf(write_file,'     // Violation 3 - Voltage of node over X pu or under Y pu for Z minutes or more\n');
        fprintf(write_file,'      node_continuous_voltage_limit_lower 0.95;\n');
        fprintf(write_file,'      node_continuous_voltage_limit_upper 1.05;\n');
        fprintf(write_file,'      node_continuous_voltage_interval 120;\n\n');

        fprintf(write_file,'      // Violation 4 / 5 - Reverse power exceeds 50%% / 75%% of the minimum trip setting of the substation breaker (fuse) or a recloser;\n');
        fprintf(write_file,'      substation_breaker_A_limit 480;\n');
        fprintf(write_file,'      substation_breaker_B_limit 480;\n');
        fprintf(write_file,'      substation_breaker_C_limit 480;\n');
        fprintf(write_file,'      virtual_substation source_10980$node_10980;\n\n');

        fprintf(write_file,'      // Violation 6 - Any voltage change at a PV POC that is greater than X perct between two Y-minute simulation time-steps\n');
        fprintf(write_file,'      inverter_v_chng_per_interval_lower_bound -0.025;\n');
        fprintf(write_file,'      inverter_v_chng_per_interval_upper_bound 0.025;\n');
        fprintf(write_file,'      inverter_v_chng_interval 60;\n\n');

        fprintf(write_file,'      // Violation 7 - X V (%%) rise/fall across the secondary distribution system\n');
...
postGeneratorScriptCleanup(fid_name,file4)
