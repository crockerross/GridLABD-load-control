...
% simulation start and end times -> please use format: yyyy-mm-dd HH:MM:SS
    ...
    data.end_date = '2009-07-02 13:08:00';    % R3: 2009-06-27  R4:2009-08-04 R5: 2009-08-21

    % How often do you want to measure?
    data.meas_interval = 2;  %applies to everything
    ...
    data.meas_limit = 1000000;%20*ceil(meas2/data.meas_interval);
...
if data.tech_flag == 0
    ...
    use_flags.use_control = 0; 
    use_flags.use_setInitTemps = 0;
    use_flags.use_baseLoadScaling = 0;
    use_flags.use_violationRecorder = 0;

    % These will include recorders/collectors/dumps
    use_flags.use_billing = 0;
    use_flags.use_emissions = 0;
    use_flags.use_capacitor_outtages = 0;
    use_flags.use_vvc = 0;
...
elseif data.tech_flag == 11
    ...
    use_flags.use_setCoolingStatus = 1;
    use_flags.use_setInitTemps = 1;  
    % --> If == 1, this will set the residential air and mass temps, 
    % commercial air and mass temps, and coil and oil temps of transformer thermal models 
    % set equal to one only if you have data from a 24hr sim
    use_flags.use_rngSetPoints = 0;
    use_flags.use_rng_allParam = 0;
    use_flags.use_xfmrThermalModel = 1;
    use_flags.use_streetlighting=1;
    use_flags.use_addBaseLoad = 0;
    use_flags.use_control = 1; 
    use_flags.use_violationRecorder = 1;
...
elseif data.tech_flag == 15
    use_flags.use_setCoolingStatusWithRng = 1;
    use_flags.use_setHvacSystemMode = 1;
    use_flags.use_setInitTemps = 1;  
    % --> If == 1, this will set the residential air and mass temps, 
    % commercial air and mass temps, and coil and oil temps of transformer thermal models 
    % set equal to one only if you have data from a 24hr sim
...
if (use_flags.use_homes == 1)
    % For residential ZIP loads ("Responsive load" and "Unresponsive load")
    % ZIP fractions and their power factors - Residential
    data.z_pf = 0.95;  % changed 1/9/2018
    data.i_pf = 0.95;
    data.p_pf = 0.95;
    data.zfrac = 0.2;
    data.ifrac = 0.4;
    ...
    % For addition of baseload
    data.baseload = 3.3;
    data.pctExtraBaseload = 0.2;
end
...
elseif (use_flags.use_commercial == 1)
    ...
    % Interior lights power factor
    data.c_z_pf_lght = 0.9;
    data.c_i_pf_lght = 0.9;
    data.c_p_pf_lght = 0.9;
